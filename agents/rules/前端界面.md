# 前端界面

如果 `src/scripts/xxx` 文件夹中既有 `index.ts` 文件也有 `index.html` 文件, 则它是一个前端界面项目.

前端界面以无沙盒 iframe 的形式在酒馆消息楼层中前台显示, 有一个自己的界面, 你可以在其中添加静态内容、样式、脚本等.

## index.html 中应该写什么

前端界面的 index.html 仅可填写静态 `<body>` 内容, 不得引用项目中其他文件, 所有非内嵌样式、代码、额外的外部依赖都应通过 Typescript 文件导入. 具体来说:

```html
<head>
  <!-- 保留一个什么都没有的 <head> 标签, Vite 打包时会在这里插入样式、脚本等 -->
</head>
<body>
  <!-- 这里写 <div>、<span> 等静态内容, 也可以只写 <div id="app"></div> 交给 vue 来渲染 -->
</body>
```

- 禁止在 `index.html` 中用 `<link rel="stylesheet" href="./index.css">` 导入样式, 而应该
  - (优先) 设计 vue 组件, 在 vue 组件中用 `<style lang="scss">` 书写.
  - 或在 Typescript 文件中用 `import './index.css'` 导入, 这样导入的样式将会经过打包最小化后插入到 `<head>` 部分;
- 禁止在 `index.html` 中用 `<script src="./index.ts">` 来引用 `index.ts` 或其他本地脚本. `index.ts` 及它导入的文件会由 Vite 直接加入到最终打包好的 `dist/scripts/**/index.html` 中.
- 在 `index.html` 中填入 `<img>` 标签时, 禁止使用 `src=""` 占位. 要么引用实际的图片, 要么不要有这个属性, 否则会导致 Vite 打包错误.

## 图标使用

你可以任意使用 fontawesome 的免费图标.

## iframe 适配要求

- 当对前端界面高度进行调整时, 禁止使用 `vh` 单位等会受宿主高度影响的单位, 而是使用 `width` 和 `aspect-ratio` 来让高度根据宽度动态调整.
- 避免使用会强制撑高父容器的元素 (如 `min-height`、`overflow: auto`).
- 页面必须有外部支撑, 主体内容不能使用 `position: absolute` 等会脱离文档流的样式.
- 页面整体应适配容器宽度，不产生横向滚动条.
- 如果样式更适合卡片形状，则不要有背景颜色，除非用户有明确要求.

## 宿主样式兼容

- 酒馆宿主会为按钮、输入框等常见元素注入全局样式, 因此即使使用了宿主现成类名如 `menu_button`, 也**不要假设**它在 iframe、扩展设置抽屉、弹窗等场景下仍会保持正确宽度或横向排版.
- 当你需要按钮横向排布、等宽占位或禁止中文按字换行时, 必须在项目自己的样式中显式覆盖关键属性, 至少检查 `display`、`width`、`min-width`、`white-space`、`writing-mode`、`text-orientation`.
- 对一组操作按钮, 应优先给外层容器显式定义 `flex` 或 `grid` 布局, 再给按钮本身定义宽度策略; 不要只依赖宿主按钮类的默认表现.
