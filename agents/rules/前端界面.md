...
- 禁止在 `index.html` 中用 `<script src="./index.ts">` 来引用 `index.ts` 或其他本地脚本. `index.ts`
  及它导入的文件会由 Vite 直接加入到最终打包好的 `dist/scripts/**/index.html` 中.
- 在 `index.html` 中填入 `<img>` 标签时, 禁止使用 `src=""`
  占位. 要么引用实际的图片, 要么不要有这个属性, 否则会导致 Vite 打包错误.
...
- 酒馆宿主会为按钮、输入框等常见元素注入全局样式, 因此即使使用了宿主现成类名如
  `menu_button`, 也**不要假设**它在 iframe、扩展设置抽屉、弹窗等场景下仍会保持正确宽度或横向排版.
- 当你需要按钮横向排布、等宽占位或禁止中文按字换行时, 必须在项目自己的样式中显式覆盖关键属性, 至少检查
  `display`、`width`、`min-width`、`white-space`、`writing-mode`、`text-orientation`.
- 对一组操作按钮, 应优先给外层容器显式定义 `flex` 或 `grid`
  布局, 再给按钮本身定义宽度策略; 不要只依赖宿主按钮类的默认表现.
- 这是高频回归项: 新增或移动 `.menu_button`
  后, 必须确认每个按钮都被项目作用域内的横排规则覆盖, 尤其是标题栏、卡片头部和动态列表中的按钮; 不能只覆盖底部操作栏. 中文按钮需实际检查不会逐字纵向换行.
