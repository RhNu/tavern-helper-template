...
如果 `src/scripts/xxx` 文件夹中仅有 `index.ts` 文件, 则它是一个脚本项目.
...
```
这种组件应该尽量参考酒馆网页原有样式; 为了让脚本里为组件设置的额外样式生效, 我们需要使用
`util/tavern-helper/script/styles.ts` 中的 `teleportStyle` 函数来将样式复制到酒馆网页的 `<head>` 中.
...
组件可能是单独的悬浮窗、手机样式的对话界面或对酒馆网页的大幅调整, 需要与酒馆网页现有样式隔离, 则应该挂载在 iframe DOM 上. 为此我们使用
`util/tavern-helper/script/host.ts` 中的 `createScriptIdIframe` 函数来创建 iframe DOM, 并将 vue 组件在 iframe load 完成后挂载到 iframe 内部的 body 上 (`iframe_element.contentDocument!.body`):
...
需要让配置与脚本变量同步时, 优先使用 `util/tavern-helper/state/script-settings.ts` 的
`createScriptSettingsSync`. 它会固定脚本 ID、校验并回写初始配置, 并提供防抖保存、立即保存及销毁前刷新能力;
不要在每次保存时重新调用 `getScriptId()` 或自行散落定时器.
