...
每个前端界面或脚本, 都以 `src/scripts` 文件夹或 `示例`
...
你可以在 `初始模板/*/新建为src文件夹中的文件夹` 中找到前端界面和脚本项目的初始模板, 复制后放入
`src/scripts`.
...
流式楼层界面即使用了第二种方法. 在 `util/ui/streaming-messages/mount.ts` 中, 项目提供了 `mountStreamingMessages`
函数来挂载流式楼层界面. 此外, 在 `示例/流式楼层界面示例` 中, 你可以找到一个流式楼层界面的示例.

**流式楼层界面不过是调用了 `mountStreamingMessages` 的脚本, 因此所有脚本的编写规则依旧适用.**
...
- `util/tavern-helper/state/mvu-store.ts` 中提供了 `defineMvuDataStore`
  函数, 它基于 Zustand 实现了本项目推荐的前端界面获取、修改 MVU 变量方式, 支持与酒馆实际变量之间的双向同步;
  `示例/角色卡示例/界面/store.ts` 中的 `useDataStore` 就是用它获取和修改界面所在楼层变量的.

你同样可以在 `初始模板/角色卡/新建为src文件夹中的文件夹` 中找到 MVU zod 角色卡的初始模板, 复制后放入
`src/scripts`.
...
`@types` 由 CI 从酒馆助手发布的 `@types.zip` 整目录同步，视为只读供应商声明，不要直接修改。上游暂缺或版本不匹配的
宿主类型应放在对应功能的适配模块旁（例如 `util/ui/popup/native-types.ts`），并只在宿主边界进行类型断言，避免同步时丢失补丁。

除了代码接口外, 酒馆自制了 STScript 命令. 要将这些命令转换为 Typescript 代码, 你需要使用 `@types/function/slash.d.ts`
内所定义的 `triggerSlash` 函数来调用它们. 具体的命令列表见于 `agents/rules/slash_command.txt` 文件.
...
- `util/core`: 不依赖酒馆宿主的集合、结构化文本、校验、日志与标识符工具
- `util/st`: SillyTavern 宿主 DOM 访问和原生命令注册
- `util/tavern-helper/script`: 脚本宿主节点、样式、生命周期与元信息
- `util/tavern-helper/state`: 酒馆变量访问、脚本配置持久化和 MVU 状态同步
- `util/ui`: 按弹窗、扩展设置、菜单、加载提示、悬浮窗和流式消息聚类；React 适配与对应功能同目录
- `src/shared`: 不独立构建、由前端脚本和后端插件按需编译的共享源码；使用 `@shared/*` 导入
- `src/shared/<领域>`: 按领域组织、由前端脚本和后端插件按需编译的共享协议、类型或客户端源码

目录与依赖边界详见 [工具模块架构.md](工具模块架构.md)。
...
如果导入的文件是 typescript、scss, 则导入的将会是经过 Vite 打包后的纯 javascript、css 而不是原始内容, 因此能在 jquery 中直接使用.
...
前端界面支持在 typescript 中 `import './index.scss'` 来导入全局 scss 文件, 并自动将它们打包到最终的
`dist/scripts/**/index.html`
中的 `<head>` 部分.
...
项目最终打包生成的 `dist/scripts/**/index.html` 或 `dist/scripts/**/index.js` 可能先上传到网上, 再以
`$('body').load(网络链接)` 或
...
`window.location.reload()`. 如聊天文件变更时重新载入前端界面或脚本, 你可以用
`util/tavern-helper/script/lifecycle.ts` 中定义好的工具函数:
